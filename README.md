...
Codexのみで開発し、私は手を加えておりません、エラーチェックは十分したつもりですが、
もし不具合等ありましたらIssuesからご報告いただけると幸いです
Windows向けビルド済みデータがあります、RAM16GB以上推奨でPyinstallerで作成してあります
...
- QRカードの検出位置を基準に、カード下部のカット数記入枠を動的にOCR
- 解析結果のサムネイル表示と、人手でのシーン名・カット数修正
- `shutil.move` によるシーン別フォルダへの動画移動と、Premiere対応XMPサイドカー作成
- UTF-8日本語QRのプレビュー、PNG保存、標準印刷、一括PNG保存
- QRコード直下へのシーン名テキスト表示（Windows日本語フォント対応）
- カット数の手書き用枠を付加したQRカードのプレビュー・印刷・一括保存
